...
usage:          import tweenMachine
                ...
                Update to Python 3 Completed by theMaize 2022.

revisions:

    - 2022.02.24 - Converted 3.0.0 from Python 2 to Python 3 to allow for future use in maya 2022 and beyond.
...
def get_maya_main_window():
    """
    Get the main Maya window widget and wrap it to a PyQt widget
    """
    main_window_ptr = omui.MQtUtil.mainWindow()  # Get the pointer to the Maya window
...
def embed_maya_window_in_pyqt(maya_parent, pyqt_widget):

    window_ptr = omui.MQtUtil.findControl("tweenMachineWindow")
    print(window_ptr)
    all_windows = mc.lsUI(windows=True)
    print(all_windows)

    maya_widget = wrapInstance(int(window_ptr), QWidget)
    if not isinstance(maya_widget, QWidget):
        print("Error: The Maya widget could not be wrapped as a QWidget.")
        return None


    maya_widget.setParent(pyqt_widget)
    maya_widget.setWindowFlags(Qt.Widget)
    maya_widget.show()
    return maya_widget

def create_pyqt_widget(maya_parent):
    pyqt_widget = QWidget()
    pyqt_widget.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
    pyqt_widget.setFixedWidth(450)
    layout = QHBoxLayout(pyqt_widget)
    layout.setContentsMargins(0, 0, 0, 0)

    maya_widget = embed_maya_window_in_pyqt(maya_parent, pyqt_widget)
    if maya_widget:
        layout.addWidget(maya_widget)
    pyqt_widget.setLayout(layout)
    #pyqt_widget.setGeometry(100, 100, 500, 300)  # Position the PyQt widget
    pyqt_widget.show()
    return pyqt_widget

SETTINGS = TMSettings()

def create_tweenmachine():
    TMWindowUI()
    maya_parent = get_maya_main_window()
    pyqt_widget = create_pyqt_widget(maya_parent)
    widget = embed_maya_window_in_pyqt(maya_parent, pyqt_widget)
    return pyqt_widget